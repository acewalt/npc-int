...
(function(){
  ...
  function classify(text){
    ...
    else if(/^(entonces )?(por )?donde empezamos$/.test(n) || /^(entonces )?(que hacemos ahora|como empezamos|y ahora que|que sigue)$/.test(n)) intent="ask_next_step";
    ...
    else if(/^(vale|ok|okay|dale|bueno|listo)$/.test(n)) intent="ack";
  ...
  function resolveReference(b){
    const fromNpc=referenceFromPreviousNpc(b);
    if(fromNpc)return fromNpc;
    return latestFocus(b);
  }
  ...
  function describeSituation(b){
    ...
    const goal=topGoal(b);
    const decision=currentDecision(b);
    const topic=latestFocus(b);
  ...
  function nextStep(b){
    ...
    if(topic){
      if(decision?.id==="ask")return `Empezaría aclarando «${short(topic,70)}». Me falta una pieza concreta de información antes de decidir algo más.`;
      if(decision?.id==="observe"||decision?.id==="investigate")return `Empezaría observando mejor «${short(topic,70)}» y buscando qué cambió, qué lo causó y qué opciones abre.`;
      return `Podemos empezar por «${short(topic,70)}». Primero separaría lo que sé, lo que estoy suponiendo y lo que todavía falta por averiguar.`;
    }

    if(goal?.label?.includes("incertidumbre"))return "Para empezar necesito una situación concreta. Dime qué está ocurriendo en el mundo del NPC; la convertiré en percepción, objetivos y opciones antes de decidir.";
    return "Empezaría definiendo un objetivo o una situación concreta del mundo. Sin eso solo tendría conversación, pero no una decisión que evaluar.";
  }

  function handleDirective(b,frame){
    const ref=resolveReference(b);
    ensureManager(b).lastResolvedReference=ref;
    if(!ref){
      return "Entiendo que quieres que haga algo, pero «lo» no tiene una referencia clara en mis últimos turnos. Dime qué quieres que conecte o pruebe y lo trataré como una acción concreta.";
    }

    const n=DMNorm(ref);
    if(/nanochat|puente neuronal|bridge|neuronal/.test(n)){
      return `Entiendo «conéctalo» como activar la conexión neuronal con ${short(ref,70)}. Esa conexión existe como acción técnica mediante /neural on; no voy a fingir que ya está conectada si el bridge local no está disponible.`;
    }
  ...
  function responseFor(b,frame){
    switch(frame.intent){
      ...
      case "directive_with_reference": return handleDirective(b,frame);
  ...
  function noteManagerTurn(b,frame,reply){
    ...
    if(reply && b.discourse){
      b.discourse.lastInterpretation={source:frame.raw,interpretation:`gestor de diálogo: ${frame.intent}`,time:b.time||0,response:reply};
    }
  }
  ...
  NpcBrain.prototype.hear=function(text){
    ...
    if(!frame.intent || frame.intent==="ack")return oldHear.call(this,text);

    const originalSay=this.say;
    this.say=function(x){return x;};
    let lower;
    try{
      lower=oldHear.call(this,text);
    }catch(err){
  ...
  print("system","","gestor de diálogo v0.1 cargado · referencias · siguiente paso · presencia · arbitraje final");
})();
