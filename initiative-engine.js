...
(function(){
  ...
  function ensure(b){if(b.initiativeEngine)return b.initiativeEngine;b.initiativeEngine={version:1,seq:1,history:[],lastCandidate:null,lastOutput:null};return b.initiativeEngine;}
  function seenRecently(b,key){const now=Date.now();return ensure(b).history.some(x=>x.key===key&&now-x.wallMs<1000*60*8);}
  ...
  function buildCandidates(b){
    ...
    if(idea?.synthesis?.claim&&idea?.focus){
      ...
      out.push(candidate(b,"share_idea",text,.78,.84,"hay una idea nueva con forma de comprobarla",{key:`idea:${norm(claim).slice(0,60)}`,topic:idea.focus}));
    }
    if(topic&&topic.mentions>1){
    ...
    if(unresolved&&active){
      ...
      out.push(candidate(b,"targeted_question",text,.60,.48,"incertidumbre concreta del tema activo",{key:`unknown:${norm(unresolved).slice(0,50)}`,topic:active.label}));
    }
  ...
  window.NpcIntInitiative={ensure,buildCandidates,choose,commit,format};
  print("system","","iniciativa social v1.0 cargada · retomar + recordar + proponer + preguntar + guardar silencio");
})();