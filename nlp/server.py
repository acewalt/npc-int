...
def semantic_frame(sentence: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not root and any(t.get("upos") == "INTJ" for t in toks):
        speech = "social"
    ...
    for si, sentence in enumerate(sentences):
        for token in sentence["tokens"]:
            if token.get("upos") in {"NOUN", "PROPN"}:
                ...
                mentions = mentions[-32:]
                continue
            if token.get("upos") != "PRON" or norm(token.get("text", "")) not in pronouns:
                continue
    ...
    for chain in getattr(doc, "coref", None) or []:
        ...
        for mi, mention in enumerate(chain.mentions):
            ...
            if isinstance(start, tuple) or isinstance(end, tuple):
                ...
                surface = " ".join(w.text for w in words)
                token_id = int(start) + 1
                start_json = int(start)
                end_json = int(end)
            mentions_json.append({
                "sentence": int(mention.sentence),
                "startWord": start_json,
                "endWord": end_json,
                "text": surface,
                "representative": mi == chain.representative_index,
            })
            if rep is not None and mi != chain.representative_index:
                ...
                refs.append({
                    "mention": surface,
                    "mentionSentence": int(mention.sentence),
                    "mentionToken": token_id,
                    "antecedent": chain.representative_text,
                    "antecedentSentence": int(rep.sentence),
                    "antecedentToken": rep_token,
                    "confidence": 0.0,
                    "source": "stanza-coref",
                    "status": "model",
                })
...
class HeuristicAnalyzer:
    ...
    coref_mode = "heuristic"
    token_re = re.compile(r"\w+(?:[-']\w+)*|[^\w\s]", re.UNICODE)
    verbs = {
        "soy": "ser", "eres": "ser", "es": "ser",
        "estoy": "estar", "estas": "estar", "estás": "estar",
        "quiero": "querer", "quieres": "querer",
        "hago": "hacer", "haces": "hacer",
        "puedo": "poder", "puedes": "poder",
        "recuerdo": "recordar", "recuerdas": "recordar",
        "entiendo": "entender", "entiendes": "entender",
    }
    ...
    def analyze(self, text: str) -> Dict[str, Any]:
        ...
        for si, piece in enumerate(pieces):
            s0 = raw.find(piece, cursor)
            cursor = max(cursor, s0 + len(piece))
            tokens = []
            root = None
            for i, m in enumerate(self.token_re.finditer(piece), 1):
                w = m.group(0)
                f = norm(w)
                upos = "PUNCT" if re.fullmatch(r"[^\w\s]", w) else "X"
                lemma = f
                feats = {}
                if f in self.pron:
                    upos = "PRON"
                ...
                tokens.append({
                    ...
                    "head": 0 if i == root else (root or 0),
                    "deprel": "root" if i == root else ("advmod" if f == "no" else "dep"),
                    "start": s0 + m.start(),
                    "end": s0 + m.end(),
                    "ner": None,
                })
            item = {"id": si, "text": piece, "tokens": tokens}
...
class StanzaAnalyzer:
    ...
    def __init__(self, model_dir: Optional[str] = None, use_gpu: bool = False, use_coref: bool = True):
        ...
        self.coref_mode = "neural" if self.coref_enabled else "heuristic-fallback"
    ...
    def spans(sentence: Any) -> Dict[int, Tuple[Optional[int], Optional[int]]]:
        ...
        for tok in sentence.tokens:
            for word in tok.words:
                ...
                out[wid] = (getattr(tok, "start_char", None), getattr(tok, "end_char", None))
        return out
    ...
    def analyze(self, text: str) -> Dict[str, Any]:
        ...
        entities = [{
            "text": e.text,
            "type": e.type,
            "start": getattr(e, "start_char", None),
            "end": getattr(e, "end_char", None),
            "source": "stanza-ner",
        } for e in (getattr(doc, "ents", []) or [])]
        sentences = []
        for si, s in enumerate(doc.sentences):
            spans = self.spans(s)
            tokens = []
            for w in s.words:
                wid = int(w.id) if isinstance(w.id, int) else int(w.id[0])
                start, end = spans.get(wid, (None, None))
                feats = feats_dict(getattr(w, "feats", None))
                ner = None
                if start is not None and end is not None:
                    hit = next((e for e in entities if e["start"] is not None and e["end"] is not None and e["start"] < end and e["end"] > start), None)
                    ner = hit["type"] if hit else None
                tokens.append({
                    "id": wid,
                    "text": w.text,
            ...
            item = {"id": si, "text": getattr(s, "text", None) or " ".join(t["text"] for t in tokens), "tokens": tokens}
            item["semanticFrame"] = semantic_frame(item)
            sentences.append(item)
        refs = []
        chains = []
        if self.coref_enabled and getattr(doc, "coref", None):
            refs, chains = stanza_coref(doc)
        if not refs:
            refs = heuristic_coref(sentences)
        return {
            "ok": True,
            ...
            "backend": self.name,
            "model": self.model,
            "processors": self.processors.split(","),
            "corefMode": "neural" if chains else "heuristic-fallback",
            "corefFallbackReason": self.coref_error,
            "text": text or "",
            "sentences": sentences,
            "entities": entities,
            "coreferences": refs,
            "coreferenceChains": chains,
            "frames": [s["semanticFrame"] for s in sentences],
        }
...
class AnalyzerRuntime:
    ...
    def load(self) -> None:
        if self.requested_backend == "heuristic":
            self.analyzer = HeuristicAnalyzer()
            return
        try:
            self.analyzer = StanzaAnalyzer(self.model_dir, self.use_gpu, self.use_coref)
        except Exception as exc:
            self.error = f"{type(exc).__name__}: {exc}"
            if self.requested_backend == "stanza":
                raise
            self.analyzer = HeuristicAnalyzer()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def reply(self, obj: Dict[str, Any], status: int = 200) -> None:
    ...
    def do_GET(self) -> None:
        if self.path.rstrip("/") == "/health":
            ...
            self.reply({
                ...
                "corefMode": getattr(a, "coref_mode", None),
                "corefFallbackReason": getattr(a, "coref_error", None),
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if not isinstance(text, str):
                self.reply({"ok": False, "error": "field 'text' must be a string"}, 400)
                return
            if len(text) > 50_000:
                self.reply({"ok": False, "error": "text too large"}, 413)
                return
            ...
            self.reply({"ok": False, "error": f"{type(exc).__name__}: {exc}"}, 500)
...
def main() -> int:
    ...
    p.add_argument("--backend", choices=["auto", "stanza", "heuristic"], default="auto")
    p.add_argument("--model-dir", default=os.environ.get("STANZA_RESOURCES_DIR"))
    p.add_argument("--gpu", action="store_true")
    p.add_argument("--no-coref", action="store_true", help="Use the lighter pipeline without neural coreference")
    args = p.parse_args()

    runtime = AnalyzerRuntime(args.backend, args.model_dir, args.gpu, not args.no_coref)
    runtime.load()
    Handler.runtime = runtime
    server = ThreadingHTTPServer((args.host, args.port), Handler)
    print(f"[nlp] http://{args.host}:{args.port} backend={runtime.backend} model={runtime.model} coref={getattr(runtime.analyzer, 'coref_mode', None)}")
