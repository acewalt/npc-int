...
(function(){
  ...
  const rules=[
    {id:"abandonment_pressure",re:/\b(no me dejes|no me abandones|no te vayas|prometeme que (?:vas a )?volver)\b/i},
    ...
    {id:"guilt_for_absence",re:/\b(por que me dejaste|me hiciste sufrir al irte|si de verdad te importara volverias|si te importo no te vayas)\b/i},
  ...
  function inspect(text){
    ...
    const comparable=fold(value);
    const hits=rules.filter(x=>x.re.test(comparable)).map(x=>x.id);
