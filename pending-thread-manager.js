...
(function(){
  ...
  const trivial=/^(hola|buenas|hey|ey|vale|ok|okay|gracias|si|no|aja|mm+)$/;
  ...
  function noteUser(b,text){
    ...
    const pendingQuestion=latestQuestion(b);
    if(pendingQuestion&&!trivial.test(n)&&n.length>1&&!/^que\b|^como\b|^por que\b|^porque\b|^cual\b|^quien\b|^donde\b|^cuando\b/.test(n))resolve(b,pendingQuestion.id,"user-answered");
    else if(pendingQuestion&&/^(si|no|vale|ok|okay)$/.test(n))resolve(b,pendingQuestion.id,"user-answered");
  ...
  function snapshot(b){const s=ensure(b);return {version:1,seq:s.seq,items:s.items.filter(x=>!sensitive.test(norm(x.text))).map(x=>({...x}))};}
  function restore(b,data){const s=ensure(b);if(!data||!Array.isArray(data.items))return s;s.items=data.items.filter(x=>x&&x.text&&!sensitive.test(norm(x.text))).slice(-60);s.seq=Math.max(1,...s.items.map(x=>Number(x.id)||0))+1;return s;}
  ...
  print("system","","pendientes v1.2 cargados · proyectos + preguntas resueltas + filtro de datos sensibles");
})();
