...
(function(){
  ...
  function candidate(text){
    const raw=String(text||"").trim(),n=norm(raw);if(!n||n.length<3||meta.test(n))return null;
    let m;
    const patterns=[
    ...
    for(const p of patterns){m=raw.match(p);if(m)return m[1].trim().replace(/[?!.]+$/g,"");}
  ...
  function noteTurn(b,text,meta={}){
    const s=ensure(b),topicText=meta.topic||candidate(text);if(!topicText)return null;
  ...
  function snapshot(b){const s=ensure(b);return {version:1,seq:s.seq,activeId:s.activeId,topics:s.topics.map(x=>({...x}))};}
  function restore(b,data){const s=ensure(b);if(!data||!Array.isArray(data.topics))return s;s.topics=data.topics.slice(-60);s.seq=Math.max(1,...s.topics.map(x=>Number(x.id)||0))+1;s.activeId=s.topics.some(x=>x.id===data.activeId)?data.activeId:null;return s;}
  ...
  print("system","","gestor de temas v1.0 cargado · temas activos/dormidos/resueltos + continuidad");
})();